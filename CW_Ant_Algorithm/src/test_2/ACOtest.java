...
//	BufferedImage bi;
    ...
    public void init(double globalBestLength) {
        ...
        for (int x = 0; x < init.N - 1; x++) {
            g2d.drawLine((int) (init.locate[result[x][0] + 1][0] * k + m),
                    (int) (init.locate[result[x][0] + 1][1] * k + m + 10),
                    (int) (init.locate[result[x][1] + 1][0] * k + m),
                    (int) (init.locate[result[x][1] + 1][1] * k + m + 10));
        }
